...
loop : Penultimate IO -> AnyCanvas -> Nat -> Nat -> Color -> IO ()
loop term canvas px py currentColor = do
  ...
  let displayCanvas = setAnyCell py px cursorCell canvas
  let uiCanvas = drawAnyText 0 0 "Arrows: Move | Space: Draw | C: Change Color | ESC: Exit" bg displayCanvas
  ...
  case key of
    ...
    KeyChar ' ' _ =>
      let newCanvas = setAnyCell py px (withBg bg currentColor) canvas
...
run = do
  ...
  loop term (emptyAnyCanvas rows cols defaultCell) (cols `div` 2) (rows `div` 2) (Named Red)
  shutdown term
