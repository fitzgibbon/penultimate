...
record RenderContext where
  ...
  lastFrameRef : IORef (Maybe Frame)
...
resolveFrame : ColorTier -> Frame -> Frame
resolveFrame tier (MkFrame width height rows) =
  MkFrame width height (map (map (resolveCell tier)) rows)
...
export
renderFrame : RenderContext -> Frame -> IO ()
renderFrame ctx frame = do
  resized <- refreshSizeIfNeeded ctx
  lastFrame <- readIORef ctx.lastFrameRef
  let resolved = resolveFrame ctx.tier frame
  let resolvedRows = rowsToList resolved.rows
  let sizeChanged = case lastFrame of
        Just prev => prev.width /= resolved.width || prev.height /= resolved.height
        Nothing => False
  if resized || sizeChanged
     then do
       writeIORef ctx.lastFrameRef Nothing
       putStr clearScreen
       putStr (cursorTo 1 1)
     else pure ()
  let output = case lastFrame of
  ...
  putStr output
  fflush stdout
  writeIORef ctx.lastFrameRef (Just resolved)
